...
const CardWrapper = ({
  ...
  const containerBase =
    variant === "auth"
      ? "min-h-[calc(100vh-4rem)] flex items-start justify-center px-3 py-6 sm:min-h-screen sm:items-center sm:px-4 sm:py-8"
      : "min-h-full flex flex-col justify-center py-10 sm:py-14 sm:px-6 lg:px-8";
  const cardBase =
    variant === "auth"
      ? "bg-white/95 dark:bg-slate-900/90 backdrop-blur-sm py-6 px-4 shadow-xl shadow-blue-500/10 rounded-2xl sm:px-10 border border-gray-200/70 dark:border-slate-800/70 max-h-[85vh] overflow-y-auto"
  ...
  return (
    <div className="relative">
      <div className={`${containerBase} ${containerClassName ?? ""}`}>
        <div className="mt-6 w-full sm:mx-auto sm:mt-8 sm:w-full sm:max-w-md">
