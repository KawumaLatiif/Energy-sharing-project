...
| File | Role |
|------|------|
| `backend/app/services/thingsboard.py` | Push telemetry + read `remaining_units` |
| `backend/app/services/electricity_ops.py` | Apply/share business logic + TB push |
| `backend/app/routers/webhooks.py` | Low-units inbound webhook |
| `backend/app/routers/meters.py` | Check-units endpoint, meter CRUD |
| `backend/app/services/email.py` | Alert email templates + deep link |
| `backend/app/services/ussd_handler.py` | USSD AMI flows |
| `docs/THINGSBOARD_WEBHOOK.md` | Webhook testing + TB rule chain notes |
| `migrations/014_notifications_and_tb_device.sql` | Notifications + optional device UUID |
| `migrations/015_user_email.sql` | User email for alerts |
